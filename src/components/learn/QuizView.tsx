...
export default function QuizView({ moduleId }: { moduleId: string }) {
  ...
  useEffect(() => {
    ...
    loadModuleAnswers(studentId, mod.slug).then((saved) => {
      setAnswers(saved);
  ...
  }, [studentId, mod?.slug]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  async function handleAnswerSubmit(questionId: string, answer: QuestionAnswer) {
    ...
    // Persist to Supabase
    if (studentId && mod) {
      await saveAnswer(studentId, mod.slug, questionId, answer);
    }
  ...
  async function handleRetake() {
    if (studentId && mod) {
      await clearModuleAnswers(studentId, mod.slug);
    }
    setAnswers({});
    setCurrentIdx(0);
  }
