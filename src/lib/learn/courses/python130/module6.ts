...
print(f"Linked list was ~{py_time/ll_time:.1f}x faster")`,
  ...
  questions: [
    ...
    },
    {
      ...
      type: 'code-challenge',
      language: 'python',
      prompt: 'Write a function count_nodes(head) that counts the number of nodes in a linked list given its head node. Then build a chain of 4 nodes (values 1, 2, 3, 4) manually using Node objects and print the count.',
      starterCode: "class Node:\n    def __init__(self, data):\n        self.data = data\n        self.next = None\n\ndef count_nodes(head):\n    pass\n\n# Build: 1 -> 2 -> 3 -> 4 -> None manually\nn1 = Node(1)\nn2 = Node(2)\nn3 = Node(3)\nn4 = Node(4)\nn1.next = n2\nn2.next = n3\nn3.next = n4\nprint(count_nodes(n1))",
      expectedOutput: "4",
      correctAnswer: '__code__',
      explanation: 'count_nodes traverses from head, incrementing a counter for each node until current is None. The chain 1→2→3→4→None has 4 nodes.',
    },
    {
      id: 'q6-15',
      type: 'code-challenge',
      language: 'python',
      prompt: 'Write a function find_last(head) that returns the data value of the last node in a linked list. Then build a chain 10 → 20 → 30 → 40 and print the last value.',
      starterCode: "class Node:\n    def __init__(self, data):\n        self.data = data\n        self.next = None\n\ndef find_last(head):\n    pass\n\nn1 = Node(10)\nn2 = Node(20)\nn3 = Node(30)\nn4 = Node(40)\nn1.next = n2\nn2.next = n3\nn3.next = n4\nprint(find_last(n1))",
      expectedOutput: "40",
      correctAnswer: '__code__',
      explanation: 'Traverse until current.next is None — that node is the last. Keep moving: current = current.next while current.next is not None, then return current.data. Returns 40 for the chain ending at 40.',
    },
    {
      id: 'q6-16',
      type: 'code-challenge',
      language: 'python',
      prompt: 'Write a function contains(head, target) that returns True if target exists in the linked list, False otherwise. Test on a chain 5 → 10 → 15 → 20. Print contains(head, 15) and contains(head, 99).',
      starterCode: "class Node:\n    def __init__(self, data):\n        self.data = data\n        self.next = None\n\ndef contains(head, target):\n    pass\n\nhead = Node(5)\nhead.next = Node(10)\nhead.next.next = Node(15)\nhead.next.next.next = Node(20)\nprint(contains(head, 15))\nprint(contains(head, 99))",
      expectedOutput: "True\nFalse",
      correctAnswer: '__code__',
      explanation: 'Traverse from head comparing each node\'s data to target. Return True immediately on a match. If you reach None without a match, return False. 15 is in the chain, 99 is not.',
    },
    {
      id: 'q6-17',
      type: 'code-challenge',
      language: 'python',
      prompt: 'Write a function print_reversed(head) that prints all node values in reverse order (from tail to head) using recursion. Test on the chain 1 → 2 → 3 → 4.',
      starterCode: "class Node:\n    def __init__(self, data):\n        self.data = data\n        self.next = None\n\ndef print_reversed(node):\n    pass\n\nhead = Node(1)\nhead.next = Node(2)\nhead.next.next = Node(3)\nhead.next.next.next = Node(4)\nprint_reversed(head)",
      expectedOutput: "4\n3\n2\n1",
      correctAnswer: '__code__',
      explanation: 'Recurse to the end first, then print on the way back: if node is None return, else recurse(node.next) then print(node.data). The recursion reaches 4 first, prints it last before unwinding to print 3, 2, 1.',
    },
    {
      id: 'q6-18',
      type: 'code-challenge',
      language: 'python',
      prompt: 'Implement a remove(item) method for LinkedList that removes the first occurrence of item. If not found, raise ValueError. Then build a list with values 5, 10, 15, 20 (using add), remove 15, and print to_list().',
      starterCode: "class Node:\n    def __init__(self, data):\n        self.data = data\n        self.next = None\n\nclass LinkedList:\n    def __init__(self):\n        self.head = None\n\n    def add(self, item):\n        node = Node(item)\n        node.next = self.head\n        self.head = node\n\n    def remove(self, item):\n        pass\n\n    def to_list(self):\n        result = []\n        current = self.head\n        while current:\n            result.append(current.data)\n            current = current.next\n        return result\n\nll = LinkedList()\nfor v in [5, 10, 15, 20]:\n    ll.add(v)\nll.remove(15)\nprint(ll.to_list())",
      expectedOutput: "[20, 10, 5]",
      correctAnswer: '__code__',
      explanation: 'add() prepends, so after adding 5,10,15,20 the list is 20→15→10→5. remove(15) uses two-pointer: previous=20, current=15 found, so 20.next = 10. Result: [20, 10, 5].',
    },
  ],
};
