...
print("3 largest:",  heapq.nlargest(3, nums))`,
  ...
  questions: [
    ...
    },
    {
      id: 'q10-4',
      type: 'fill-in-blank',
      prompt: 'In a binary heap stored as a list with root at index 1, the left child of the node at index 3 is at index ___',
      correctAnswer: '6',
      explanation: 'The index formula for a 1-indexed heap: left child at 2*i, right child at 2*i+1, parent at i//2. For i=3: left child = 2*3 = 6. Starting at index 1 (not 0) makes these formulas work cleanly.',
    },
    {
      id: 'q10-5',
      type: 'multiple-choice',
    ...
    },
    {
      ...
      type: 'code-challenge',
      language: 'python',
      prompt: 'Implement a MinHeap class with insert(val) and delete_min() operations. Use a list with index 1 as the root (index 0 is unused). insert appends to the end then percolates up by swapping with the parent (at i//2) while smaller. delete_min moves the last element to the root, removes the last slot, then percolates down by swapping with the smaller child.\n\nExample:\nInsert 5, 3, 8, 1, 4\ndelete_min() twice\nprint(heap[1:]) → [4, 5, 8]',
      starterCode: "class MinHeap:\n    def __init__(self):\n        self.heap = [0]  # index 0 unused, root is at index 1\n\n    def insert(self, val):\n        pass\n\n    def delete_min(self):\n        pass\n\nh = MinHeap()\nfor v in [5, 3, 8, 1, 4]:\n    h.insert(v)\nh.delete_min()\nh.delete_min()\nprint(h.heap[1:])",
      expectedOutput: "[4, 5, 8]",
      correctAnswer: '__code__',
      explanation: 'insert: append val, then while i > 1 and heap[i] < heap[i//2], swap and move i = i//2. delete_min: save heap[1], move heap[-1] to heap[1], pop last, then percolate down by swapping with the smaller child. After inserting [5,3,8,1,4] the heap is [1,3,8,5,4]. First delete_min removes 1 → heap becomes [3,4,8,5]. Second removes 3 → heap becomes [4,5,8].',
    },
    {
      id: 'q10-13',
      type: 'code-challenge',
      language: 'python',
      prompt: 'Use Python\'s heapq module to implement a function `top_k(lst, k)` that returns the k smallest elements from a list, sorted ascending.\n\nExample:\ntop_k([7, 1, 5, 3, 9, 2, 8, 4, 6], 3) → [1, 2, 3]',
    ...
    },
    {
      ...
      type: 'code-challenge',
      language: 'python',
      prompt: 'Use heapq to write a function `find_median(nums)` that returns the median of a list of numbers. If the list has an odd length return the middle value; if even return the average of the two middle values. Use heapq to sort the list.\n\nExample:\nfind_median([3, 1, 4, 1, 5]) → 3\nfind_median([1, 2, 3, 4]) → 2.5',
      starterCode: "import heapq\n\ndef find_median(nums):\n    pass\n\nprint(find_median([3, 1, 4, 1, 5]))\nprint(find_median([1, 2, 3, 4]))",
      expectedOutput: "3\n2.5",
      correctAnswer: '__code__',
      explanation: 'Heapify a copy, then pop all elements to get them sorted. If len is odd, the median is the middle element (sorted[n//2]). If even, average the two middle elements ((sorted[n//2-1] + sorted[n//2]) / 2). find_median([3,1,4,1,5]) sorted → [1,1,3,4,5], middle = 3. find_median([1,2,3,4]) → (2+3)/2 = 2.5.',
    },
    {
      id: 'q10-15',
      type: 'code-challenge',
      language: 'python',
      prompt: 'Implement heap sort using Python\'s heapq. Write `heap_sort(lst)` that returns a new sorted list by heapifying a copy and repeatedly popping the minimum.\n\nExample:\nheap_sort([5, 3, 8, 1, 9, 2, 7, 4, 6]) → [1, 2, 3, 4, 5, 6, 7, 8, 9]',
      starterCode: "import heapq\n\ndef heap_sort(lst):\n    # Make a copy and heapify it\n    # Pop elements one by one into a result list\n    pass\n\nprint(heap_sort([5, 3, 8, 1, 9, 2, 7, 4, 6]))",
      expectedOutput: "[1, 2, 3, 4, 5, 6, 7, 8, 9]",
      correctAnswer: '__code__',
      explanation: 'h = lst[:]; heapq.heapify(h); return [heapq.heappop(h) for _ in range(len(h))]. heapify is O(n), then n pops each O(log n) → O(n log n) total.',
    },
    {
      id: 'q10-17',
      type: 'code-challenge',
      language: 'python',
      prompt: 'Write a function `kth_smallest(lst, k)` that uses heapq to return the k-th smallest element in a list (1-indexed, so k=1 is the smallest).\n\nExample:\nkth_smallest([7, 2, 5, 1, 8, 3], 3) → 3\n(sorted: [1, 2, 3, 5, 7, 8] — 3rd smallest is 3)',
      starterCode: "import heapq\n\ndef kth_smallest(lst, k):\n    pass\n\nprint(kth_smallest([7, 2, 5, 1, 8, 3], 3))",
      expectedOutput: "3",
      correctAnswer: '__code__',
      explanation: 'heapify a copy of the list, then heappop k times. The k-th pop gives the k-th smallest. h = lst[:]; heapq.heapify(h); for _ in range(k-1): heapq.heappop(h); return heapq.heappop(h).',
    },
    {
      id: 'q10-18',
      type: 'code-challenge',
      language: 'python',
      prompt: 'Write a function `max_k_pops(lst, k)` that uses a max-heap (negate trick) to pop and return the k largest elements from a list, in descending order.\n\nExample:\nmax_k_pops([4, 1, 7, 3, 9, 2], 3) → [9, 7, 4]',
      starterCode: "import heapq\n\ndef max_k_pops(lst, k):\n    pass\n\nprint(max_k_pops([4, 1, 7, 3, 9, 2], 3))",
      expectedOutput: "[9, 7, 4]",
      correctAnswer: '__code__',
      explanation: 'Build a max-heap by negating: h = [-x for x in lst]; heapq.heapify(h). Then pop k times, negating each result back: return [-heapq.heappop(h) for _ in range(k)]. Popping from a max-heap gives the largest first.',
    },
  ],
};
