...
print("3 largest:",  heapq.nlargest(3, nums))`,
  ...
  questions: [
    ...
    },
    {
      ...
      type: 'code-challenge',
      language: 'python',
      prompt: 'Implement a MinHeap class with insert(val) and delete_min() operations. Use a list with index 1 as root. insert adds to end and percolates up; delete_min swaps root with last element, removes last, and percolates down.\n\nExample:\nInsert 5, 3, 8, 1, 4 → heap is [1, 3, 8, 5, 4]\ndelete_min() → removes 1\ndelete_min() → removes 3\nprint(heap[1:]) → [3, 5, 8, 4]',
    ...
    },
    {
      ...
      type: 'code-challenge',
      language: 'python',
      prompt: 'Use heapq to implement a function `merge_sorted(lists)` that merges k sorted lists into one sorted list. Use a heap of (value, list_index, element_index) tuples.\n\nExample:\nmerge_sorted([[1,4,7],[2,5,8],[3,6,9]]) → [1, 2, 3, 4, 5, 6, 7, 8, 9]',
      ...
      correctAnswer: '__code__',
      explanation: 'After popping (val, list_idx, elem_idx), check if elem_idx + 1 < len(lists[list_idx]) and push (lists[list_idx][elem_idx+1], list_idx, elem_idx+1). The heap always holds the current smallest unprocessed element from each list — O(n log k) overall.',
    },
    {
      id: 'q10-15',
      type: 'code-challenge',
    ...
    },
    {
      ...
      type: 'code-challenge',
      language: 'python',
      prompt: 'Write a function `running_median(nums)` that returns a list where each element is the median of all numbers seen so far. Use two heaps: a max-heap (negate values) for the lower half and a min-heap for the upper half.\n\nExample:\nrunning_median([5, 2, 4, 1, 6]) → [5, 3.5, 4, 3.0, 4]\nAfter 5     → median = 5\nAfter 5,2   → median = 3.5\nAfter 5,2,4 → median = 4',
      starterCode: "import heapq\n\ndef running_median(nums):\n    lower = []  # max-heap (negate values)\n    upper = []  # min-heap\n    result = []\n    for num in nums:\n        # Push to lower (max-heap), then balance\n        heapq.heappush(lower, -num)\n        heapq.heappush(upper, -heapq.heappop(lower))\n        if len(upper) > len(lower):\n            heapq.heappush(lower, -heapq.heappop(upper))\n        # Compute median\n        pass\n    return result\n\nprint(running_median([5, 2, 4, 1, 6]))",
      expectedOutput: "[5, 3.5, 4, 3.0, 4]",
      correctAnswer: '__code__',
      explanation: 'If both halves are equal size, median = (-lower[0] + upper[0]) / 2. If lower is larger, median = -lower[0]. After [5]: lower=[-5] → 5. After [5,2]: lower=[-2], upper=[5] → 3.5. After [5,2,4]: lower=[-4,-2], upper=[5] → 4. After [5,2,4,1]: lower=[-2,-1], upper=[4,5] → 3.0. After [5,2,4,1,6]: lower=[-4,-2,-1], upper=[5,6] → 4.',
    },
    {
      id: 'q10-17',
      type: 'code-challenge',
