...
print("After:", root.in_order())    # [1, 6, 8, 10, 13, 14]`,
  ...
  questions: [
    ...
    },
    {
      ...
      type: 'code-challenge',
      language: 'python',
      prompt: 'Implement a BST with insert(value), search(value), and in_order() methods. Insert 8, 3, 10, 1, 6, 14, 13 (in that order). Then search for 6 and 7, printing results. Also print an in-order traversal to verify the BST is sorted.',
    ...
    },
    {
      ...
      correctAnswer: '__code__',
      explanation: 'bst_from_sorted recursively builds balanced halves: mid = len(lst)//2, root = BST(lst[mid]), then recursively insert left half and right half. In-order traversal gives back the sorted list.',
    },
    {
      id: 'q8-14',
      type: 'code-challenge',
      language: 'python',
      prompt: 'Write a function is_valid_bst(node, min_val, max_val) that returns True if the tree rooted at node is a valid BST. Call it with float(\'-inf\') and float(\'inf\') as bounds. Test on a valid BST (insert 8,3,10,1,6) and a manually broken one where a node violates the property.',
      starterCode: "class BST:\n    def __init__(self, data):\n        self.data = data\n        self.left = None\n        self.right = None\n    def insert(self, value):\n        if value < self.data:\n            if self.left is None: self.left = BST(value)\n            else: self.left.insert(value)\n        else:\n            if self.right is None: self.right = BST(value)\n            else: self.right.insert(value)\n\ndef is_valid_bst(node, min_val, max_val):\n    pass\n\nroot = BST(8)\nfor v in [3, 10, 1, 6]: root.insert(v)\nprint(is_valid_bst(root, float('-inf'), float('inf')))\n\n# Break the BST property manually\nroot.left.right.right = BST(99)  # 99 > 8 but sits in left subtree\nprint(is_valid_bst(root, float('-inf'), float('inf')))",
      expectedOutput: "True\nFalse",
      correctAnswer: '__code__',
      explanation: 'is_valid_bst: if node is None return True, if not (min_val < node.data < max_val) return False, else recurse with updated bounds: left gets max_val=node.data, right gets min_val=node.data. Inserting 99 into the left subtree violates the upper bound of 8.',
    },
    {
      id: 'q8-15',
      type: 'code-challenge',
      language: 'python',
      prompt: 'Write a function bst_height(node) that returns the height of a BST. Then compare the height of a BST built from random order [5,3,7,1,4,6,8] vs sorted order [1,2,3,4,5,6,7]. Print both heights.',
      starterCode: "class BST:\n    def __init__(self, data):\n        self.data = data\n        self.left = None\n        self.right = None\n    def insert(self, value):\n        if value < self.data:\n            if self.left is None: self.left = BST(value)\n            else: self.left.insert(value)\n        else:\n            if self.right is None: self.right = BST(value)\n            else: self.right.insert(value)\n\ndef bst_height(node):\n    pass\n\nbalanced = BST(5)\nfor v in [3, 7, 1, 4, 6, 8]: balanced.insert(v)\n\ndegenerate = BST(1)\nfor v in [2, 3, 4, 5, 6, 7]: degenerate.insert(v)\n\nprint(bst_height(balanced))\nprint(bst_height(degenerate))",
      expectedOutput: "3\n7",
      correctAnswer: '__code__',
      explanation: 'bst_height: if node is None return 0, else 1 + max(bst_height(left), bst_height(right)). The balanced BST has height 3 (log2(7)≈3). The degenerate BST from sorted input has height 7 — one long chain.',
    },
    {
      id: 'q8-16',
      type: 'code-challenge',
      language: 'python',
      prompt: 'Write a function count_greater(root, k) that returns how many values in a BST are strictly greater than k. Use the BST property to avoid visiting unnecessary nodes. Test on BST with values [8,3,10,1,6,14,13], k=8.',
      starterCode: "class BST:\n    def __init__(self, data):\n        self.data = data\n        self.left = None\n        self.right = None\n    def insert(self, value):\n        if value < self.data:\n            if self.left is None: self.left = BST(value)\n            else: self.left.insert(value)\n        else:\n            if self.right is None: self.right = BST(value)\n            else: self.right.insert(value)\n\ndef count_greater(node, k):\n    pass\n\nroot = BST(8)\nfor v in [3, 10, 1, 6, 14, 13]: root.insert(v)\nprint(count_greater(root, 8))",
      expectedOutput: "3",
      correctAnswer: '__code__',
      explanation: 'count_greater: if node is None return 0. If node.data > k: count 1 + count_greater(left, k) + count_greater(right, k) — both subtrees may have values >k. If node.data <= k: only right subtree can have values >k. Values greater than 8 are: 10, 14, 13 — count is 3.',
    },
    {
      id: 'q8-17',
      type: 'code-challenge',
      language: 'python',
      prompt: 'Implement delete(value) on BST handling all 3 cases: leaf, one child, two children (use in-order successor). Build BST from [8,3,10,1,6,14,13], delete 3 (two children), then print in_order().',
      starterCode: "class BST:\n    def __init__(self, data):\n        self.data = data\n        self.left = None\n        self.right = None\n    def insert(self, value):\n        if value < self.data:\n            if self.left is None: self.left = BST(value)\n            else: self.left.insert(value)\n        else:\n            if self.right is None: self.right = BST(value)\n            else: self.right.insert(value)\n    def in_order(self):\n        result = []\n        if self.left: result += self.left.in_order()\n        result.append(self.data)\n        if self.right: result += self.right.in_order()\n        return result\n    def find_min(self):\n        current = self\n        while current.left: current = current.left\n        return current.data\n\n    def delete(self, value):\n        pass\n\nroot = BST(8)\nfor v in [3, 10, 1, 6, 14, 13]: root.insert(v)\nroot = root.delete(3)\nprint(root.in_order())",
      expectedOutput: "[1, 6, 8, 10, 13, 14]",
      correctAnswer: '__code__',
      explanation: 'delete: navigate to the node. If leaf return None. If one child return that child. If two children: find in-order successor (right.find_min()), replace data, delete successor from right subtree. Deleting 3 (two children: 1 and 6): successor is 6, replace 3 with 6, delete 6 from right subtree.',
    },
    {
      id: 'q8-18',
      type: 'code-challenge',
      language: 'python',
      prompt: 'Write a function find_min(root) that returns the minimum value in a BST. Then write find_max(root) that returns the maximum. Test on a BST built from [8, 3, 10, 1, 6, 14]. Print the min and max.',
      starterCode: "class BST:\n    def __init__(self, data):\n        self.data = data\n        self.left = None\n        self.right = None\n    def insert(self, value):\n        if value < self.data:\n            if self.left is None: self.left = BST(value)\n            else: self.left.insert(value)\n        else:\n            if self.right is None: self.right = BST(value)\n            else: self.right.insert(value)\n\ndef find_min(root):\n    pass\n\ndef find_max(root):\n    pass\n\nroot = BST(8)\nfor v in [3, 10, 1, 6, 14]:\n    root.insert(v)\nprint(find_min(root))\nprint(find_max(root))",
      expectedOutput: "1\n14",
      correctAnswer: '__code__',
      explanation: 'find_min follows .left repeatedly until .left is None, returning that node\'s data. find_max follows .right. In a BST with values 1,3,6,8,10,14: min is 1 (leftmost), max is 14 (rightmost).',
    },
  ],
};
