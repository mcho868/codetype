...
print("Result:", evaluate(root))  # (3+4) * (2-1) = 7`,
  ...
  questions: [
    ...
    },
    {
      ...
      type: 'code-challenge',
      language: 'python',
      prompt: 'Write a recursive function count_nodes(node) that returns the total number of nodes in a binary tree. Test on a tree: root=1, left=2, right=3, 2.left=4, 2.right=5. Print the count.',
      starterCode: "class BinaryTree:\n    def __init__(self, data):\n        self.data = data\n        self.left = None\n        self.right = None\n\ndef count_nodes(node):\n    pass\n\nroot = BinaryTree(1)\nroot.left = BinaryTree(2)\nroot.right = BinaryTree(3)\nroot.left.left = BinaryTree(4)\nroot.left.right = BinaryTree(5)\nprint(count_nodes(root))",
      expectedOutput: "5",
      correctAnswer: '__code__',
      explanation: 'count_nodes: if node is None return 0, else return 1 + count_nodes(node.left) + count_nodes(node.right). The tree has 5 nodes total: 1, 2, 3, 4, 5.',
    },
    {
      id: 'q7-15',
      type: 'code-challenge',
      language: 'python',
      prompt: 'Write a recursive function find_max(node) that returns the maximum value in a binary tree (not a BST — values can be anywhere). Test on a tree: root=3, left=7, right=1, 7.left=9, 7.right=2.',
      starterCode: "class BinaryTree:\n    def __init__(self, data):\n        self.data = data\n        self.left = None\n        self.right = None\n\ndef find_max(node):\n    pass\n\nroot = BinaryTree(3)\nroot.left = BinaryTree(7)\nroot.right = BinaryTree(1)\nroot.left.left = BinaryTree(9)\nroot.left.right = BinaryTree(2)\nprint(find_max(root))",
      expectedOutput: "9",
      correctAnswer: '__code__',
      explanation: 'find_max: if node is None return float(\'-inf\'), else return max(node.data, find_max(node.left), find_max(node.right)). Checks all nodes since there\'s no ordering property. Returns 9.',
    },
    {
      id: 'q7-16',
      type: 'code-challenge',
      language: 'python',
      prompt: 'Write a function is_leaf(node) that returns True if the node has no children. Then write count_leaves(node) that counts all leaf nodes in the tree. Test on: root=1, left=2, right=3, 2.left=4, 2.right=5. Print count_leaves.',
      starterCode: "class BinaryTree:\n    def __init__(self, data):\n        self.data = data\n        self.left = None\n        self.right = None\n\ndef is_leaf(node):\n    pass\n\ndef count_leaves(node):\n    pass\n\nroot = BinaryTree(1)\nroot.left = BinaryTree(2)\nroot.right = BinaryTree(3)\nroot.left.left = BinaryTree(4)\nroot.left.right = BinaryTree(5)\nprint(count_leaves(root))",
      expectedOutput: "3",
      correctAnswer: '__code__',
      explanation: 'is_leaf: node.left is None and node.right is None. count_leaves: if node is None return 0, if is_leaf return 1, else return count_leaves(left) + count_leaves(right). Leaves are 3, 4, 5 — count is 3.',
    },
    {
      id: 'q7-17',
      type: 'code-challenge',
      language: 'python',
      prompt: 'Write a function level_sum(root, level) that returns the sum of all node values at a given depth level (root is level 0). Test on: root=1, left=2, right=3, 2.left=4, 2.right=5. Print level_sum(root, 2).',
      starterCode: "class BinaryTree:\n    def __init__(self, data):\n        self.data = data\n        self.left = None\n        self.right = None\n\ndef level_sum(node, level):\n    pass\n\nroot = BinaryTree(1)\nroot.left = BinaryTree(2)\nroot.right = BinaryTree(3)\nroot.left.left = BinaryTree(4)\nroot.left.right = BinaryTree(5)\nprint(level_sum(root, 2))",
      expectedOutput: "9",
      correctAnswer: '__code__',
      explanation: 'level_sum: if node is None return 0, if level == 0 return node.data, else return level_sum(node.left, level-1) + level_sum(node.right, level-1). Level 2 nodes are 4 and 5 — sum is 4+5=9.',
    },
    {
      id: 'q7-18',
      type: 'code-challenge',
      language: 'python',
      prompt: 'Write an in_order(node) and post_order(node) function that each print node data. Use the tree: root=1, left=2, right=3, 2.left=4, 2.right=5. Call both functions and observe the different output orders.',
      starterCode: "class BinaryTree:\n    def __init__(self, data):\n        self.data = data\n        self.left = None\n        self.right = None\n\ndef in_order(node):\n    pass\n\ndef post_order(node):\n    pass\n\nroot = BinaryTree(1)\nroot.left = BinaryTree(2)\nroot.right = BinaryTree(3)\nroot.left.left = BinaryTree(4)\nroot.left.right = BinaryTree(5)\nin_order(root)\npost_order(root)",
      expectedOutput: "4\n2\n5\n1\n3\n4\n5\n2\n3\n1",
      correctAnswer: '__code__',
      explanation: 'In-order (L→N→R): 4, 2, 5, 1, 3. Post-order (L→R→N): 4, 5, 2, 3, 1. Both visit all 5 nodes but in different sequences determined by when the current node is printed relative to its children.',
    },
  ],
};
